...
export default function Slider({ testimonials }) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto text-center">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[480px] h-[480px] pointer-events-none before:absolute before:inset-0 before:bg-gradient-to-b before:from-indigo-500/25 before:via-indigo-500/5 before:via-25% before:to-indigo-500/0 before:to-75% before:rounded-full before:-z-10">
          <div className="h-48 [mask-image:_linear-gradient(0deg,transparent,theme(colors.white)_20%,theme(colors.white))]">
            {testimonials.map((testimonial, index) => (
              ...
              >
                <Image
                  className="relative -translate-x-1/2 rounded-full top-1 left-1/2"
                  src={testimonial.img}
                  width={200}
                  height={200}
      ...
      {/* Text */}
      <div className="transition-all duration-150 ease-in-out delay-300 mb-9">
        <div className="relative flex flex-col mt-14" ref={testimonialsRef}>
          {testimonials.map((testimonial, index) => (
            <Transition
              key={index}
              show={active === index}
            ...
            >
              <div className="text-lg font-bold text-white before:content-['\201C'] after:content-['\201D']">
                {testimonial.quote}
              </div>
            </Transition>
          ))}
